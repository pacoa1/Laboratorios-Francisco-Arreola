...
13. [Lectura 13: Normalización](#lectura-13-normalización)
...
## Lectura 13: Normalización

### Resumen

La normalización es el proceso de diseñar y simplificar la estructura de una
base de datos relacional para eliminar la redundancia de información y prevenir
inconsistencias (anomalías). Una base de datos bien diseñada es estable, más
fácil de actualizar y tiene una mayor esperanza de vida frente a nuevos
requerimientos. Este proceso se basa en aplicar reglas secuenciales llamadas
"Formas Normales". La Primera Forma Normal (1FN) exige que cada celda contenga
valores atómicos (un solo valor, sin grupos repetidos) y que no existan filas
idénticas. La Segunda Forma Normal (2FN) requiere estar en 1FN y que todos los
atributos que no son llaves dependan por completo de la llave primaria. La
Tercera Forma Normal (3FN) exige estar en 2FN y eliminar las dependencias
transitivas, es decir, que un atributo no llave dependa de otro atributo que
tampoco es llave. Existen niveles más avanzados para situaciones complejas: la
Forma Normal de Boyce-Codd (BCNF) hace más estricta la 3FN exigiendo que cada
determinante sea forzosamente una llave candidata; la Cuarta Forma Normal (4FN)
elimina las dependencias de valores múltiples separando datos independientes en
dos tablas distintas para evitar repeticiones masivas; y la Quinta Forma Normal
(5FN) trata con dependencias de producto para evitar dividir tablas de una
forma que luego impida reconstruir la información original. En conclusión,
normalizar es desarmar tablas problemáticas en tablas más pequeñas y
específicas siguiendo reglas matemáticas estrictas.

### Preguntas

Pregunta 1: ¿Qué condición principal debe cumplir obligatoriamente una tabla para que se considere que ya está en la Primera Forma Normal (1FN)?
Pregunta 2: ¿Cuál es la diferencia exacta entre la Tercera Forma Normal (3FN) y la Forma Normal de Boyce-Codd (BCNF) con respecto al uso de los determinantes?
